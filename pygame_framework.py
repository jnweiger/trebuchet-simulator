#!/usr/bin/python
#
# (C) Copyright 2011, jw@suse.de, 
# merged test_main and pygame_main into pygame_framework.
# 
...
class Framework(object):
    ...
    * Handles all user input.
    ...
    def Step(self, settings):
        ...
        if settings.hz > 0.0:
            ### use 0.1 for extreme slow slow motion
            ### use 1.0 for real time, which is not exact enough for the fast trebuchet.
            timeStep = 1.0 / (settings.slowMotion * settings.hz)
        else:
            timeStep = 0.0
    ...
    def _Keyboard_Event(self, key):
        ...
        """
        if key==K_z or key==K_PLUS:       # Zoom in
            if self.viewZoom < 300:
              self.viewCenter.x -= .3*self.viewZoom
              self.setCenter(self.viewCenter)
              self.viewZoom = 1.1 * self.viewZoom
        elif key==K_x or key==K_MINUS:     # Zoom out
            if self.viewZoom > 0.02:
              self.viewCenter.x += .3*self.viewZoom
              self.setCenter(self.viewCenter)
              self.viewZoom = 0.9 * self.viewZoom
